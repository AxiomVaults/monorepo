...
-- ══ CONFIRMED FROM P ══
-- Recent data (Jan-Mar 2026): dex_rate ~1.11, fair_value ~1.09-1.17, spread ±500 bps — clean
-- Oct-Dec 2025 data: garbage dex_rates (95M, 9K, 257, 118). Root cause: cartesian join
--   artifacts from txs with multiple inflows OR outflows in same tx (LP rebalancing,
--   multi-hop arb bots). Fix: filter to txs with exactly 1 inflow AND 1 outflow = Query Q.
-- ─────────────────────────────────────────────────────────────────────────────


-- ══ QUERY Q: Weekly DEX rate vs fair value — CLEAN (1:1 swap filter) ══
-- Adds clean_tx CTE: only keeps txs where exactly 1 token flows in AND 1 flows out.
-- Eliminates LP add/remove events, multi-hop arb bots, and cartesian join artifacts.
...
-- Only keep txs with exactly 1 distinct token flowing in AND 1 flowing out = clean swap
clean_tx AS (
