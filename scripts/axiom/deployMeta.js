// scripts/axiom/deployMeta.js
// Deploy the Axiom meta-vault system on Flow EVM mainnet (chainId 747).
//
// What this deploys (in order):
//   1.  AxiomVault               — ERC-4626 capital owner
//   2.  MultiStrategyManager     — multi-adapter yield router
...
//
// Usage:
//   node_modules/.bin/hardhat run scripts/axiom/deployMeta.js --network flow_mainnet
//
// Requires: PRIVATE_KEY_FLOW set in .env
// Output saved to scripts/axiom/deployed-mainnet.json
...
const CONFIG = {
  ...
  moreLending: {},
  punchSwapLP: { maxSlippageBps: 200 },
  redemption:  { claimDelay: 604_800, maxSlippageBps: 100 }, // 7 days — matches Ankr unbonding queue
  pair: { discountBps: 20 },

  // Initial APY hints (bps) — updated live by keeper bot
  apy: {
...
async function main() {
  ...
  // Guard: only run on Flow EVM mainnet
  if (network.chainId !== 747n) {
    throw new Error(`Expected chainId 747 (Flow EVM mainnet), got ${network.chainId}. Use --network flow_mainnet`);
  }

  console.log("\n╔══════════════════════════════════════════════════════════════╗");
  console.log("║         Axiom Meta-Vault Deploy — Flow EVM Mainnet           ║");
  console.log("╚══════════════════════════════════════════════════════════════╝");
  ...
  if (bal === 0n) {
    throw new Error("Deployer balance is 0 — fund the deployer wallet before deploying");
  }
  ...
  // Vault: grant roles to MultiStrategyManager
  ...
  // Transfer adapter ownership to MultiStrategyManager
  ...
  // AnkrYieldAdapter / AnkrMOREYieldAdapter: grant OPERATOR_ROLE to MultiStrategyManager
  try {
    console.log("» AnkrYieldAdapter: grant OPERATOR_ROLE → MultiStrategyManager");
    await (await ayaC.grantRole(OPERATOR_ROLE, d.multiStrategyManager)).wait();

    console.log("» AnkrMOREYieldAdapter: grant OPERATOR_ROLE → MultiStrategyManager");
    await (await amyaC.grantRole(OPERATOR_ROLE, d.multiStrategyManager)).wait();
  } catch {
  ...
  // MultiStrategyManager: grant VENUE_ROLE to AxiomVenue
  ...
  console.log("\n─── Next Steps ──────────────────────────────────────────────");
  console.log("  1. Copy deployed-mainnet.json to scripts/axiom/keeper/");
  console.log("  2. Run grant-keeper-role.js to give keeper OPERATOR_ROLE");
  console.log("  3. Update dapp/.env and dapp/lib/contracts.ts with vault address");
  console.log("  4. Start keeper bot:  DEPLOYED_JSON=deployed-mainnet.json node keeperBot.js");
  ...
  const outPath = path.join(__dirname, "deployed-mainnet.json");
